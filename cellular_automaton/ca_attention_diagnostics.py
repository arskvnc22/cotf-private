"""Validate inputs for a cellular-automaton attention diagnostic run."""
...
DEFAULT_OUTPUT_ROOT = Path("/scratch/tak1e25/ca-attention-diagnostics")
...
def resolve_model_args(manifest, attention_implementation=None):
    ...
    model_args.model = "ca_cotf_cache_attn"
...
def validate_source_run(args):
    ...
    supported_models = {"ca_cotf", "ca_cotf_cache_attn"}
    if model_config.get("model") not in supported_models:
        raise ValueError(
            "expected source model 'ca_cotf' or 'ca_cotf_cache_attn', "
