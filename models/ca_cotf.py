...
class CausalSelfAttention(nn.Module): # should be able to use bidirectional as well in the future not now though
    ...
    def forward(self, x, pos_emb_closure, cache_context, start_index, indices): # indices seem to be a leftover from ACT variants, unused in fixed depth models
...
class GPTBase(nn.Module):
    ...
    def forward(self, idx, targets=None, get_logits=False, use_cache=False, iter=None, return_all_logits=False,num_repeats=None,return_repeat_states=False):
        device = idx.device
        b, t = idx.size()
