...
class GPTBase(nn.Module):
    ...
    def __init__(self, config):
        ...
        if config.direction_embedding == True:
    ...
    def forward(
        ...
        if return_repeat_logits and len(self.transformer.h_end) != 0:
            raise ValueError(
                "Per-repeat decoding currently requires n_layer_end == 0."
            )
        ...
        for rep_idx in range(1, repeats + 1):
            if self.config.direction_embedding == True:

                direction= direction_schedule[:,rep_idx -1]
                x=self.direction_embedding(x,direction)
